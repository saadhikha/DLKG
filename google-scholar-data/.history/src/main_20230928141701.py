...
def parse_paper_element(paper_element):
  ...
  for author in authors_element.children:
if isinstance(child, NavigableString) and child.strip():  # Checking for text node
        author['name'] = child.strip()
    elif child.name == 'a':  # Checking for <a> tag
        author['name'] = child.text
        author['link'] = child['href']
