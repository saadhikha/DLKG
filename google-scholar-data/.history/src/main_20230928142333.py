...
def parse_paper_element(paper_element):
  ...
  for author_element in authors_element.children:
    if isinstance(author_element, NavigableString) and author_element.strip(): 
        name = author_element.strip()
        profile_link = None
    elif author_element.name == 'a':  # Checking for <a> tag
        name = author_element.text
        profile_link = author_element['href']

    author_id = generate_author_id(name, profile_link)
    all_authors[author_id] = { "name": name, profile_link }
